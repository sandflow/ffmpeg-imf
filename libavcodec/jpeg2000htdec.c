                                            ...
                                            const uint8_t *array)
{
    ...
    /*
     * We are reading bytes from end to start, we need
     * to be careful about how we handle us being close to the end
     * Subtracting by 4 means we will read some of the bytes of
     * the MEL byte stream since the MEL byte stream
     * ends at the start of the VLC byte stream.
     * This is okay as they are masked away since we check for
     * cases where that occurs(when the position is less than 4).
     */
    ...
    if (buffer->pos < 4){
        ...
        uint64_t mask = (1ul << (buffer->pos + 1) * 8) - 1;
        tmp &= mask;
    }
